...
export default async function HistoryDetailPage({
	...
	const yearData = MOCK_HISTORY_DATA.find((d) => d.year === yearNum);
	const monthlyData = yearData?.months.find((m) => m.month === monthNum);
		...
		return (
			<div className="max-w-3xl mx-auto px-4 py-16 text-center space-y-6">
				...
				<p className="text-slate-400 text-sm">
					指定された年月（{yearNum}年{monthNum}月）の履歴データは存在しません。
				</p>
				<Link
					href="/history"
	...
	const categorySpentMap = expenses.reduce(
		(acc, curr) => {
								...
								{budgets.map((category) => {
									...
									const percentage =
										category.budget > 0
											...
											: 0;
									const isCategoryOver = spent > category.budget;
